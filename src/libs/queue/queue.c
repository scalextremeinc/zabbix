...
void queue_ctx_init(struct queue_ctx* ctx, const char* recovery_dir) {
    ...
    char* recovery_file = (char*) malloc((500 + len) * sizeof(char));
    memcpy(recovery_file, recovery_dir, len);
    char hostname[400];
    gethostname(hostname, 400);
    zbx_snprintf(recovery_file + len, 500, "/zabbix-%s-%d.recovery", hostname, (int) getpid());
    //sprintf(recovery_file + len, "/zabbix-%s-%d.recovery", hostname, (int) getpid());
...
void queue_sock_connect_msg(struct queue_ctx* ctx, const char* queue_addr_msg) {
    ...
    //int hwm = 0;
    //if (zmq_setsockopt(ctx->zmq_sock_msg, ZMQ_SNDHWM, &hwm, sizeof(hwm)) == -1) {
    //    zabbix_log(LOG_LEVEL_ERR, "Error setting zmq high watter mark on msg sock: %s",
    //        strerror(errno));
    //} 
...
void queue_msg_send_error(struct queue_ctx* ctx, const char* msg) {
    if (ctx->zmq_sock_err != NULL) {
        if (queue_msg_send(ctx->zmq_sock_err, "Zabbix error sending message") == -1) {
    ...
    update_recovery(ctx, msg);
}

void update_recovery(struct queue_ctx* ctx, const char* msg) {
	...
	while (SUCCEED == ret && NULL != (p = zbx_json_next(&jp_data, p)))	{
        ...
        if (SUCCEED == zbx_json_value_by_name_dyn(&jp_row, ZBX_PROTO_TAG_CLOCK, &tmp, &tmp_alloc)) {
            ...
            update_recovery_entry(ctx, ts, new_gap);
            new_gap = 0;
        }
    }
    zbx_free(tmp);
...
void update_recovery_entry(struct queue_ctx* ctx, long ts, int new_gap) {
    int ts1, ts2;
    uint32_t net;
    struct flock fl;
    ...
    if (new_gap || size == 0) {
        ...
        zabbix_log(LOG_LEVEL_DEBUG, "Current gap extends2, ts1: %d, ts2: %d", ts1, ts2);
    }
